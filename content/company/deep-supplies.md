---
name: Deep Supplies
date: '2025-02-20'
overview: Reacting to the problems we have seen over the past 25 years in the defence, maritime and oil and gas industries; Deep Supplies realised that there is an intelligent way of collecting all types of data. Having a team of qualified and very experienced operators; we have built our business on the ethic of providing reliable information first time. Using the latest available technology, systems and software we can get you the answers you need quickly. We can collect the data you want. GPS, bathymetric, acoustic, personnel monitoring and positioning and any data you specifically need. We have worked with the innovation centre on Garden Island WA introducing subsea devices for monitoring and inspection purposes. Deep supplies have also worked very closely with onshore and offshore companies supplying equipment unique to the required industry; offering the right system first time. We are currently developing an AI system for auto recognition of dangers such as shark identification, bush
  fire growth/direction and a personnel danger vicinity monitoring system.
website: https://deep.supplies
contact_name: Mark Wanless
contact_title: Director
address: 3/14 Beneficial Way,, Wangara WA 6065
phone: 0488005080
email: sales@deep.supplies
company_types:
- sme
capability_domains:
- armour
- intelligence
- maritime
- multi
- reconnaissance
- science
- search
- special
- stable
- submarines
- surveillance
industrial_capabilities:
- composites
- hydraulics
- mechanical
capability_streams:
- electronic
- land
latitude: -31.8175278
longitude: 115.7984507
---

## Overview

Reacting to the problems we have seen over the past 25 years in the defence, maritime and oil and gas industries; Deep Supplies realised that there is an intelligent way of collecting all types of data. Having a team of qualified and very experienced operators; we have built our business on the ethic of providing reliable information first time. Using the latest available technology, systems and software we can get you the answers you need quickly. We can collect the data you want. GPS, bathymetric, acoustic, personnel monitoring and positioning and any data you specifically need. We have worked with the innovation centre on Garden Island WA introducing subsea devices for monitoring and inspection purposes. Deep supplies have also worked very closely with onshore and offshore companies supplying equipment unique to the required industry; offering the right system first time. We are currently developing an AI system for auto recognition of dangers such as shark identification, bush fire growth/direction and a personnel danger vicinity monitoring system.
## Capabilities

Deep Supplies brings the latest technically advanced system to Australia and New Zealand. Our American/ European approved systems with modular components can collect a wide variety of data; either below the sea, on the surface and up to 5000m above sea level. With a range of sensors collecting acoustic data, photogrammetry images, live personnel monitoring and/or a live video feedback to command centres. Our products can save lives and time. Most systems are deployed in under 5 minutes and can have up to 12 hours standby times. Our systems can be used not only for defence situations, but for detection of invasive marine species, search and rescue, surveillance, maritime support, reconnaissance, special operations, subsea/aerial drug concealments, poaching, bush fire monitoring and marine movement. If we do not have the system, we can build a system that can.
