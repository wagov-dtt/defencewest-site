---
name: 'Touchtech Australia '
date: '2025-02-20'
overview: 'Touchtech Australia Pty Ltd (Touchtech) is a locally owned Bunbury based company that focuses on software and product development to solve business concerns for our clients. '
website: https://touchtech.com.au
contact_name: Chris Dickie
contact_title: Director
address: 6 Beddingfield Street, Davenport WA 6230
phone: 0438931911
email: chris@touchtech.com.au
company_types:
- sme
capability_domains:
- communications
- land
- light
- maritime
- multi
- science
- search
- utility
industrial_capabilities:
- design
- electronics
- protective
regions:
- south
capability_streams:
- electronic
- land
latitude: -33.3555756
longitude: 115.678321
---

## Overview

Touchtech Australia Pty Ltd (Touchtech) is a locally owned Bunbury based company that focuses on software and product development to solve business concerns for our clients.\
Touchtech has a range of products that we can customise to meet individual client needs, and can develop bespoke software and products specifically to suit a client. Clients have included small business, mining, transport & earth moving, commercial fishing and government.
## Capabilities

Touchtech maintains software and electronics design and maintenance capabilities so that we can rapidly respond to changing priorities for our clients. Touchtech is an authorised Solution Provider with ORBCOMM, a leading satellite service provider. Touchtech is able to program ORBCOMM hardware and provide end-to-end solutions with hardware and airtime incorporating satellite and mobile data solutions. Other branded mobile and satellite hardware is available and incorporated into projects as best fits client scenarios. Touchtech provides complete solutions, including hardware such as tablets and vehicle computers, with communications using mobile or satellite data. Touchtech uses Garmin Fleet hardware in a range of systems.
